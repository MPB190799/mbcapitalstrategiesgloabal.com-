...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Restore previous consent on page load
    if (consentState === 'accepted' && typeof gtag === 'function') {
        gtag('consent', 'update', {
            'ad_storage': 'granted',
    ...
    // CMP-Stacking-Guard v2 (web-design-agent 2026-06-13):
    // Do NOT show banner simultaneously with Google Funding Choices CMP (double-dialog = UX-kill).
    // Polls for TCF-API / googlefc for up to 3.5s before showing own banner.
    if (cookieBanner && !consentState) {
        let _cmpAttempts = 0;
        function _checkCmpBlog() {
            _cmpAttempts++;
            ...
            if (typeof window.__tcfapi === 'function') {
                window.__tcfapi('addEventListener', 2, function(d, ok) {
                    if (ok && (d.eventStatus === 'useractioncomplete' || d.eventStatus === 'tcloaded')) {
                        if (!localStorage.getItem('cookie_consent')) cookieBanner.classList.add('show');
                    }
                });
                return;
            }
            if (_cmpAttempts < 35) { setTimeout(_checkCmpBlog, 100); return; }
            if (!localStorage.getItem('cookie_consent')) cookieBanner.classList.add('show');
        }
        setTimeout(_checkCmpBlog, 200);
    }
    ...
    // Revoke consent function (called from privacy page button)
